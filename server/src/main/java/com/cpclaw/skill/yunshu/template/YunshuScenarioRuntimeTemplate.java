...
/** Optional scenario runtime template; not a default Spring service. */
public class YunshuScenarioRuntimeTemplate {
    ...
    private final YunshuScenarioTemplateSemantics questionSemantics;

    public YunshuScenarioRuntimeTemplate(
        ...
        YunshuScenarioTemplateSemantics questionSemantics
    ) {
    ...
    public CloudPivotQueryAnswer query(MetadataSearchResult match, String userQuestion, String modelConfigId, boolean thinkingEnabled, List<RuntimeQueryFilter> filters, List<String> metricFieldCodes, Map<String, Object> reasoningContext, AgentProgressListener progressListener) {
        ...
        String password = readableUserPassword();
        ...
        progress.onExecution(
            "获取“" + match.name() + "”数据",
            "业务数据读取完成。",
            Map.of("entityName", match.name()),
            "completed"
        );
        progress.onExecution(
            "核验数据范围",
            "已取得数据，总数=" + result.total() + "，本次参与处理=" + result.records().size() + " 条。",
            Map.of("total", result.total(), "returnedRecords", result.records().size()),
            "completed"
        );
        ...
        String answer = ensureConclusionHeading(detail.answer());
        ...
        return new CloudPivotQueryAnswer(
            ...
            (isDetailCollectionQuestion(userQuestion) || isSingleRecordDetailQuestion(userQuestion)) ? List.copyOf(result.records()) : List.of()
        );
    }
    ...
    private String readableUserPassword() {
        try {
    ...
    private String streamModelAnswer(
        ...
        if (modelAnswer.isPresent() && !modelAnswer.get().isBlank()) {
            ...
            return appendVerifiedRecordEvidence(modelAnswer.get(), result);
        }
    ...
    private String ensureConclusionHeading(String answer) {
        String value = answer == null ? "" : answer;
        return value.contains("结论摘要") ? value : "### 结论摘要\n" + value;
    }

    /**
     * Model prose is not itself evidence.  Preserve a small, display-policy
     * filtered sample so a read response remains inspectable even when the
     * model elected to discuss only aggregates.  Field selection and masking
     * are owned by the Yunshu Skill display policy.
     */
    private String appendVerifiedRecordEvidence(String answer, CloudPivotRuntimeQueryResult result) {
        String normalized = answer == null ? "" : answer;
        if (!normalized.contains("结论摘要")) {
            normalized = "### 结论摘要\n" + normalized;
        }
        if (result.records().isEmpty()) return normalized;
        List<String> samples = result.records().stream()
            .limit(3)
            .map(record -> recordSummary(result.schemaCode(), record))
            .filter(value -> value != null && !value.isBlank() && !"无可展示字段".equals(value))
            .distinct()
            .toList();
        if (samples.isEmpty()) return normalized;
        return normalized + "\n\n### 已核验记录示例\n" + samples.stream()
            .map(value -> "- " + value)
    ...
    private RuntimeAnswerDetail amountAggregationAnalysis(MetadataSearchResult match, CloudPivotRuntimeQueryResult result, List<String> metricFieldCodes, List<RuntimeQueryFilter> filters) {
        ...
        if (amounts.isEmpty()) {
            ...
            answer.append("\n### 处理口径\n");
            answer.append("- 已继承或匹配云枢元数据对象“").append(match.name()).append("”。\n");
        ...
        answer.append("\n### 处理口径\n");
        answer.append("- 识别业务对象：").append(match.name()).append("。\n");
    ...
    private RuntimeAnswerDetail rankingAnalysis(MetadataSearchResult match, CloudPivotRuntimeQueryResult result, RuntimeQuestionPlan answerPlan, String userQuestion, String modelConfigId, boolean thinkingEnabled, Map<String, Object> reasoningContext) {
        ...
        // A ranking is already a fully determined result from the same record
        // snapshot. Do not append an unconstrained LLM narrative which can deny
        // the verified values displayed immediately above.
        return new RuntimeAnswerDetail(answer.toString(), "指标排行：按回答计划识别指标字段并降序返回重点记录", ruleConclusion);
    }
    ...
    private RuntimeAnswerDetail statusAmountAggregationAnalysis(MetadataSearchResult match, CloudPivotRuntimeQueryResult result, String userQuestion) {
        ...
        answer.append("\n\n### 处理口径");
        answer.append("\n- 继承或匹配云枢元数据对象“").append(match.name()).append("”。");
    ...
    private boolean isCountQuestion(String content) {
        return questionSemantics.isCountQuestion(content);
    }

    private boolean isAnalysisQuestion(String content) {
        return questionSemantics.isAnalysisQuestion(content);
    }

    private boolean isDetailCollectionQuestion(String content) {
        return questionSemantics.isDetailCollectionQuestion(content);
    }

    private boolean isSingleRecordDetailQuestion(String content) {
        return questionSemantics.isSingleRecordDetailQuestion(content);
    }

    private int requestedRecordOrdinal(String content) {
        return questionSemantics.requestedRecordOrdinal(content);
    }
    ...
    private boolean isNewOldCustomerQuestion(String content) {
        return questionSemantics.isLifecycleComparisonQuestion(content);
    }

    private boolean isOwnerOpportunityRankingQuestion(String content) {
        return questionSemantics.isOwnerRankingQuestion(content);
    }

    private boolean isStatusAmountAggregationQuestion(String content) {
        return questionSemantics.isStatusMetricAggregationQuestion(content);
    }

    private boolean isAmountAggregationQuestion(String content) {
        return questionSemantics.isMetricAggregationQuestion(content);
    }

    private RuntimeQuestionPlan planRuntimeQuestion(String content) {
        com.cpclaw.skill.SkillQuestionSemantics.QuestionPlan plan = questionSemantics.plan(content);
        RuntimeMetric metric = "AMOUNT".equals(plan.metric()) ? RuntimeMetric.AMOUNT : RuntimeMetric.NONE;
        RuntimeOperation operation = switch (plan.operation()) {
            case "RANKING" -> RuntimeOperation.RANKING;
            case "AGGREGATION" -> RuntimeOperation.AGGREGATION;
            default -> RuntimeOperation.DEFAULT;
        };
        return new RuntimeQuestionPlan(operation, metric, operation == RuntimeOperation.RANKING ? RuntimeSort.DESC : RuntimeSort.NONE, plan.limit());
    }

    private RuntimeMetric detectMetric(String value) {
        return "AMOUNT".equals(questionSemantics.plan(value).metric()) ? RuntimeMetric.AMOUNT : RuntimeMetric.NONE;
    }

    private RuntimeOperation detectOperation(String value, RuntimeMetric metric) {
        return switch (questionSemantics.plan(value).operation()) {
            case "RANKING" -> RuntimeOperation.RANKING;
            case "AGGREGATION" -> RuntimeOperation.AGGREGATION;
            default -> RuntimeOperation.DEFAULT;
        };
    ...
    private boolean isAmountRankingQuestion(String content) {
        return questionSemantics.isMetricRankingQuestion(content);
    }

    private boolean asksRanking(String value) {
        return questionSemantics.isMetricRankingQuestion(value);
    }

    private boolean asksAggregation(String value) {
        return questionSemantics.isMetricAggregationQuestion(value);
    }
    ...
    private List<String> requestedStatusFilters(String content) {
        return questionSemantics.requestedStatusFilters(content);
    }

    private boolean statusMatches(String actualStatus, List<String> targetStatuses) {
        return questionSemantics.statusMatches(actualStatus, targetStatuses);
    }

    private boolean isStageDistributionQuestion(String content) {
        return questionSemantics.isStageDistributionQuestion(content);
    }

    private boolean isYearlyDistributionQuestion(String content) {
        return questionSemantics.isYearlyDistributionQuestion(content);
    }

    private boolean isProvinceDistributionQuestion(String content) {
        return questionSemantics.isProvinceDistributionQuestion(content);
    }
    ...
    private int queryPageSize(String content) {
        CloudPivotRuntimeProperties.Query query = runtimeProperties.getQuery();
        return questionSemantics.queryPageSize(content, query, requestedOwnerFilter(content).present());
    }

    private int queryRecordLimit(String content) {
        CloudPivotRuntimeProperties.Query query = runtimeProperties.getQuery();
        return questionSemantics.queryRecordLimit(content, query, requestedOwnerFilter(content).present());
    }

    private boolean requiresCompleteAggregation(String content) {
        return questionSemantics.requiresCompleteAggregation(content);
    }

    private boolean requiresFullDimensionDetails(String content) {
        return questionSemantics.requiresFullDimensionDetails(content);
    }

    private boolean isBroadBusinessAnalysisQuestion(String content) {
        return questionSemantics.isBroadBusinessAnalysisQuestion(content);
    }

    private boolean isPlainListQuestion(String content) {
        return questionSemantics.isPlainListQuestion(content);
    }
