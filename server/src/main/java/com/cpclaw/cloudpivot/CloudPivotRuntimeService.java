...
public class CloudPivotRuntimeService {
    ...
    private RuntimeAnswerDetail recordDetailList(MetadataSearchResult match, CloudPivotRuntimeQueryResult result) {
        ...
        for (int i = 0; i < result.records().size(); i++) {
            ...
            Object displayName = firstNonBlank(data, "instanceName", "name", "title", "标题", "商机名称", "客户名称");
            answer.append("\n\n").append(i + 1).append(". ").append(displayName == null ? "未命名记录" : valueText(displayName));
            appendField(answer, "记录ID", record.get("id"));
            appendField(answer, "阶段/状态", firstNonBlank(data, "stage", "阶段", "status", "状态", "projectStatus", "项目状态"));
            appendField(answer, "金额", firstNonBlank(data, "amount", "金额", "opportunityAmount", "商机金额", "projectAmount", "项目金额", "合同额"));
            appendField(answer, "负责人", firstNonBlank(data, "owner", "负责人", "ownerName", "createdByName"));
            appendField(answer, "客户", firstNonBlank(data, "customer", "客户", "customerName", "cust_id", "opportunityCustomer"));
            appendField(answer, "创建时间", firstNonBlank(data, "createdAt", "createTime", "createdTime", "创建时间"));
            appendAdditionalFields(answer, data);
        }
    ...
    private RuntimeAnswerDetail singleRecordDetail(MetadataSearchResult match, CloudPivotRuntimeQueryResult result, String userQuestion) {
        ...
        appendField(answer, "记录ID", record.get("id"));
        appendField(answer, "名称", firstNonBlank(data, "instanceName", "name", "title", "标题", "商机名称", "客户名称"));
        appendField(answer, "阶段", firstNonBlank(data, "stage", "阶段", "status", "状态"));
        appendField(answer, "金额", firstNonBlank(data, "amount", "金额", "opportunityAmount", "商机金额"));
        appendField(answer, "负责人", firstNonBlank(data, "owner", "负责人", "ownerName", "createdByName"));
        appendField(answer, "客户", firstNonBlank(data, "customer", "客户", "customerName", "cust_id", "opportunityCustomer"));
        appendField(answer, "创建时间", firstNonBlank(data, "createdAt", "createTime", "createdTime", "创建时间"));
        appendAdditionalFields(answer, data);
    ...
    private RuntimeAnswerDetail rankingAnalysis(MetadataSearchResult match, CloudPivotRuntimeQueryResult result, RuntimeQuestionPlan answerPlan, String userQuestion, String modelConfigId, boolean thinkingEnabled, Map<String, Object> reasoningContext) {
        ...
        if (rankedRecords.isEmpty()) {
            ...
            if (!result.records().isEmpty()) {
                answer.append("\n返回样本：").append(recordSummary(result.records().getFirst())).append("。\n");
            }
    ...
    private int queryRecordLimit(String content) {
        if (requestedOwnerFilter(content).present()) {
            return requiresCompleteAggregation(content) ? 20_000 : 50;
        }
        if (isSingleRecordDetailQuestion(content)) {
            return requestedRecordOrdinal(content);
        }
