...
function compactTimeline(steps: ExecutionStep[]) {
  const deduplicated = collapseLifecycleSteps(steps)

  const hasRichUnderstanding = deduplicated.some((step) => (
    stepKind(step) !== 'progress' && timelineStage(step) === 'understand'
  ))
  const hasCompletedTask = deduplicated.some((step) => isTaskCompletion(step))
  const richStages = new Set(
    deduplicated
      .filter((step) => stepKind(step) !== 'progress')
      .map(timelineStage)
      .filter(Boolean)
  )

  return deduplicated.filter((step) => {
    if (!props.compact && hasRichUnderstanding && isQuestionReceipt(step)) {
      return false
    }
    if (hasCompletedTask && isAnswerTransition(step)) {
      return false
    }
    if (stepKind(step) !== 'progress') {
      return true
    }
    if (isTaskCompletion(step) || (props.compact && isQuestionReceipt(step))) {
      return true
    }
    const stage = timelineStage(step)
    return !isGenericProgress(step) || !stage || !richStages.has(stage)
  })
}
...
function isGenericProgress(step: ExecutionStep) {
  return stepKind(step) === 'progress'
    && ['理解问题', '规划路径', '查询数据', '整理回答'].includes(step.title)
}

function isQuestionReceipt(step: ExecutionStep) {
  return step.title === '理解问题' || /已收到问题/.test(step.status)
}

function isAnswerTransition(step: ExecutionStep) {
  return step.title === '正式回答' || step.phase === 'answer_start' || step.phase === 'answer_end'
}

function isTaskCompletion(step: ExecutionStep) {
  return step.title === '任务完成' || step.phase === 'end'
}

function timelineStage(step: ExecutionStep) {
  const value = `${step.phase || ''} ${step.title} ${step.status}`
  if (/understand|收到问题|理解/.test(value)) return 'understand'
  if (/plan|规划|元数据/.test(value)) return 'plan'
  if (/查询|接口|数据返回|经营指标|业务元数据图/.test(value)) return 'execute'
  if (/answer|回答|总结|校验|整理/.test(value)) return 'answer'
  return ''
}
