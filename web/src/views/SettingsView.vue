...
onMounted(() => { syncSectionFromRoute(); void loadSettings(); window.addEventListener('keydown', handleSettingsEscape) }); onBeforeUnmount(() => window.removeEventListener('keydown', handleSettingsEscape)); watch(() => [route.path, route.query.section], syncSectionFromRoute); watch(activeSection, (section) => { if (route.path === '/settings' && route.query.section !== section) void router.replace({ query: { ...route.query, section } }) }); watch(() => [userForm.cloudPivotUsername, userForm.cloudPivotPassword], () => { userFormVersion.value += 1 }, { flush: 'sync' }); watch(() => [adminForm.targetBaseUrl, adminForm.username, adminForm.password], () => { adminFormVersion.value += 1 }, { flush: 'sync' }); watch(() => [modelForm.modelName, modelForm.modelApiBaseUrl, modelForm.modelApiKey], () => { modelFormVersion.value += 1 }, { flush: 'sync' })
