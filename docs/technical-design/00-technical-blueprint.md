...
6. AI 模型调用统一采用 OpenAI 兼容格式，通过系统设置配置模型。
7. 高风险操作由后端确认机制拦截，不依赖模型自觉。
...
封装 OpenAI 兼容模型调用。模型 API 地址、API Key、模型名称、是否支持思考模式、默认思考模式来自系统设置。API Key 运行时解密，仅用于请求 Header，不写日志。

### 4.4 Metadata Service

负责从云枢同步元数据，标准化为应用级知识图谱，生成检索文档和 embedding 文本，写入 MySQL 和 Elasticsearch/OpenSearch。MySQL 是权威主库，检索索引可重建。
