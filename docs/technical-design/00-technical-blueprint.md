...
3. 当前阶段先不做向量化语义检索；优先基于 MySQL Metadata Index 实现精确名称、别名、编码、应用路径、全文关键词和复合业务词等非向量检索。后续必要时再扩展 Elasticsearch/OpenSearch 和 Milvus。
...
负责在管理员授权下连接目标云枢环境并同步元数据，标准化为应用级知识图谱，生成可解释检索文档并写入 MySQL Metadata Index。MySQL 是权威主库，Elasticsearch/OpenSearch 和向量索引属于后续可重建增强项。
...
当前阶段先基于 MySQL Metadata Index 实现确定性、可解释、可测试的非向量检索：
...
4. 规则融合排序：综合对象类型、名称命中、显式应用路径、真实 CRM 核心对象保护、辅助对象降权、上下文和记忆权重。
5. Explanation：输出匹配原因、业务关键词、目标对象、`graphPath`、分数和召回依据，供前端 Markdown 展示。
6. 后续增强：必要时再接入 Elasticsearch/OpenSearch 的 BM25、模型重排和 Milvus/Embedding 向量召回；向量检索不得在当前阶段决定核心业务对象。
