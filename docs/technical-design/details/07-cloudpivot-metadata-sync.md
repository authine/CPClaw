...
同步后的应用、业务模型和数据项必须保存可供 Agent 使用的中文语义说明。云枢接口已提供 `description`、`remark`、`helpText` 或 `tips` 时优先保留；缺失时由云枢 Skill 的元数据语义层生成中性说明，不根据“商机”“项目”等特定业务对象编造含义。

数据项增加 `field_category`：`ownerDeptId`、`modifiedTime`、`name`、`createdDeptId`、`creater`、`sequenceNo`、`createdTime`、`modifier`、`owner`、`workflowInstanceId`、`processInstanceId` 及云枢流程/记录生命周期状态字段为 `SYSTEM`；其余数据项为 `BUSINESS`。这是云枢 Skill 的提供方元数据规则，不属于 CPClaw 框架层的业务判断。

元数据展示保留云枢原始 `dataType` 编码到 `dataTypeCode`，同时由云枢 Skill 提供 `dataType` 的人类可读类型名称，避免界面直接暴露 `0`、`3`、`50`、`60` 等底层枚举值。

数据洞察默认从 `BUSINESS` 字段选择阶段、金额、业务状态和分析维度；`SYSTEM` 字段不参与这些业务维度的推断。创建/修改时间、拥有者等系统字段仅在用户明确要求时间范围、当前用户范围或审计口径时作为辅助筛选字段使用。
