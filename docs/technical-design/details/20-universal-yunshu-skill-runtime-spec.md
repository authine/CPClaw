...
CPClaw 建设一个可被多个宿主复用的“统一云枢 Skill”。该 Skill 不把云枢能力实现为一组分散的原子工具，也不把具体业务场景写死在 Prompt 或框架代码中，而是定义一套通用的语义理解、元数据驱动、受控执行和结果交付规范。
...
CPClaw Web/Agent 直接调用 `TaskGateway`，由 Runtime 统一执行。会话记忆、用户记忆和系统全局记忆只作为受控上下文输入，不改变权限和执行计划。
