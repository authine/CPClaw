...
MVP 规则型实现中，低置信度或缺少可查询对象时统一进入 `clarify_intent`。澄清回复必须包含：系统已理解到的内容、当前缺失的信息、可补充的方向，以及本地可见的部分候选对象。
...
2. 从本地 Metadata Index 匹配业务对象，例如“商机”。
3. 调用云枢运行态查询数据。
4. 优先调用用户配置的 OpenAI-compatible 模型进行分析。
5. 模型不可用时返回规则分析兜底，至少包括结论摘要、关键发现、风险信号和下一步建议。
6. 如果对象未匹配，进入澄清模式，说明已理解为分析类请求并引导用户补充对象。
