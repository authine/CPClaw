...
右侧“后端处理流程”面板以助手消息为入口恢复执行链：当前会话新回复优先使用前端内存中的 `AgentResponse.steps`；刷新或打开历史会话后，前端从助手消息 metadata 读取 `agentRunId`，调用 `/api/audit/agent-runs/{id}` 懒加载 `plan_json`、`reflection_json` 和工具调用记录，再重建 Observe、Think、Act、Reflect 摘要。历史消息若没有 `agentRunId`，但仍有 `source=runtime-query`、`schemaCode`、total 等旧 metadata，只能展示有限运行态摘要，并提示新消息才支持完整流程恢复。

这些内容是对执行链的可审计摘要，不是模型不可验证的隐藏思维。默认对话流不展示“执行过程”块；需要排查“商机/客户对象是否串线”“统计总数是否来自正确 schema”等问题时，应从审计页、Agent Run 和工具调用记录查看。

助手正文使用 Markdown 作为标准格式，前端必须解析并安全渲染标题、列表、加粗、代码和表格。历史消息中如果已经包含旧版 `### 执行过程` 前置块，前端展示层需要兼容隐藏，只保留业务结论部分。

长耗时任务的前端反馈必须先于后端最终返回。当前 MVP 尚未完成真正 SSE 流式响应，因此采用“本地临时流式状态”兜底：用户提交后，前端立即插入一条不落库的 CPClaw 占位回复，并在右侧流程面板显示临时 Observe、Think、Act、Reflect 摘要；后端返回后再用真实助手消息、真实步骤和真实审计信息替换占位内容。临时状态只能描述工程进度，例如“正在读取上下文、正在识别阶段分布追问、正在等待云枢运行态接口返回”，不能伪造真实 `schemaCode`、工具结果或模型结论。
