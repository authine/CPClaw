...
为了让用户能判断 Agent 是否真的理解了问题，读数据类回答必须输出可审计执行过程摘要，而不是只返回最终数字。当前 MVP 的回答正文会展示：

- 意图理解：例如计数查询、列表查询、按年分析、业务分析。
- 对象匹配：命中的对象名称和编码。真实云枢查询展示真实 `schemaCode`；本地 fallback 演示查询必须展示为“演示编码”，并明确说明不是真实云枢编码。
- 数据动作：调用的运行态查询、来源端点、总数和本次返回记录数。
- 原始数据摘要：返回记录的前几条可读字段摘要，不展示密码、Token 等敏感字段。
- 结论生成：说明最终数字或分析结论如何从返回数据得出。

这些内容是对执行链的可审计摘要，不是模型不可验证的隐藏思维。前端“后端处理流程”卡片也应展示同样口径的步骤状态，便于排查“商机/客户对象是否串线”“统计总数是否来自正确 schema”等问题。在 `sourceEndpoint=local-fallback` 时，任何 `system_*` 编码都只能解释为本地演示编码，不能对用户声称是真实云枢对象编码。
...
示例：用户说“系统有多少商机？”，Agent 应执行：意图识别为 `query_data`，Query rewrite 后保留业务词“商机”，匹配“系统商机”或真实 CRM 商机实体，调用云枢运行态查询并返回总数。只有对象无法匹配或统计条件本身会导致明显查错时，才进入澄清。

示例：用户说“系统有多少个客户？”，Agent 应执行：意图识别为 `query_data`，业务对象识别为“客户”，匹配“系统客户”或真实客户实体，调用云枢运行态查询并直接返回客户总数，不应要求用户补充应用或表单。

计数回答必须在执行过程里明确编码来源和原始数据摘要。真实云枢环境下，商机和客户应显示真实 `schemaCode`；本地 fallback 演示环境下，应显示 `演示编码=system_opportunity` 或 `演示编码=system_customer`，并提示“不是真实云枢 schemaCode”。如果两个问题返回相同编码或相同原始摘要，应视为回归缺陷。
...
4. 调用云枢运行态查询数据；分析类请求应尽量分页拉取完整数据，当前 MVP 设置保护上限，避免只拿第一页样本就给出全量结论。
...
示例：用户说“分析系统中的商机信息”，Agent 应识别为 `analyze_data`，匹配商机相关实体，先查询商机数据，再生成分析结论，而不是要求用户重新说明“云枢对象”。
