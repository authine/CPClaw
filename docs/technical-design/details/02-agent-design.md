...
计数回答必须在执行过程里明确 schema 和原始数据摘要，例如商机应显示 `schemaCode=system_opportunity`、总数 3，客户应显示 `schemaCode=system_customer`、总数 4；如果两个问题返回相同 schema 或相同原始摘要，应视为回归缺陷。
