...
10. 规则融合排序：融合名称、编码、应用路径、对象类型、真实主对象保护、辅助对象降权、同步时间和记忆权重；当前不使用向量分数。
11. Explanation：生成匹配原因。

向量化语义检索暂不作为当前实现路径。后续如接入 Elasticsearch/OpenSearch、Embedding 或 Milvus，只能作为可解释召回的增强来源，不能绕过真实云枢 Metadata Index、图谱校验和确认机制。
